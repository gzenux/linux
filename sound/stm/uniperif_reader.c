...
static void uniperif_reader_parse_dt(struct platform_device *pdev,
...
static int uniperif_reader_probe(struct platform_device *pdev)
{
	...
	if (!pdev->dev.of_node)
		...
		uniperif_reader_parse_dt(pdev, reader);
...
static int uniperif_reader_resume(struct device *dev)
{
	...
	dev_dbg(dev, "%s(dev=%p)", __func__, dev);
	if (reader->pads)
		stm_pad_setup(reader->pads);
	snd_power_change_state(card, SNDRV_CTL_POWER_D0);

	return 0;
}

static const struct dev_pm_ops uniperif_reader_pm_ops = {
	.suspend = uniperif_reader_suspend,
	.resume	 = uniperif_reader_resume,
	.freeze	 = uniperif_reader_suspend,
	.restore = uniperif_reader_resume,
	.thaw	 = uniperif_reader_resume,
};
#endif
...
static struct platform_driver uniperif_reader_platform_driver = {
	...
	.driver.of_match_table = of_match_ptr(uniperif_reader_match),
#ifdef CONFIG_PM
