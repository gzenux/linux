...
static int uniperif_player_close(struct snd_pcm_substream *substream)
{
	...
	/* Just return when player not stopped (i.e parking/standby) */
...
static void uniperif_player_parse_dt(struct platform_device *pdev,
...
static int uniperif_player_probe(struct platform_device *pdev)
{
	...
	if (!pdev->dev.of_node)
		...
		uniperif_player_parse_dt(pdev, player);
...
static int uniperif_player_resume(struct device *dev)
{
	...
	if (player->pads)
		stm_pad_setup(player->pads);
	snd_power_change_state(card, SNDRV_CTL_POWER_D0);

	return 0;
}

static const struct dev_pm_ops uniperif_player_pm_ops = {
	.suspend = uniperif_player_suspend,
	.freeze	 = uniperif_player_suspend,
	.resume	 = uniperif_player_resume,
	.thaw	 = uniperif_player_resume,
	.restore = uniperif_player_resume,
};
#endif
...
static struct platform_driver uniperif_player_platform_driver = {
	...
	.driver.of_match_table = of_match_ptr(uniperif_player_match),
#ifdef CONFIG_PM
