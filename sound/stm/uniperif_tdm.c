...
static void uniperif_tdm_parse_dt(struct platform_device *pdev,
		struct uniperif_tdm *tdm)
{
	struct snd_stm_uniperif_tdm_info *info;
	struct device_node *pnode;
...
static int uniperif_tdm_probe(struct platform_device *pdev)
{
	...
	if (!pdev->dev.of_node)
		...
		uniperif_tdm_parse_dt(pdev, tdm);
...
static struct platform_driver uniperif_tdm_platform_driver = {
	...
	.driver.of_match_table = of_match_ptr(uniperif_tdm_match),
#ifdef CONFIG_PM
