...
#ifdef CONFIG_OF
static int stm_keypad_matrix_key_parse_dt(struct stm_keyscan_priv *priv)
{
	struct device *dev = priv->input->dev.parent;
	struct device_node *np = dev->of_node;
	struct stm_keyscan_config *pdata;
	int error;
	...
	of_property_read_u32(np, "st,keyscan-num-rows", &pdata->num_in_pads);
	of_property_read_u32(np, "st,keyscan-num-columns",
							&pdata->num_out_pads);

	if (!pdata->num_in_pads || !pdata->num_out_pads) {
		dev_err(dev, "missed info for keyscan\n");
		return -EINVAL;
	}
	...
	error = of_property_read_u32_array(np, "linux,keymap",
					pdata->keycodes, STM_KEYSCAN_MAXKEYS);

	if (error) {
		dev_err(dev, "missed keymap for keyscan\n");
		return -EINVAL;
	}
...
static int stm_keyscan_init_conf(struct device *dev,
			struct stm_pad_config *pad_config, const char *owner)
{
#ifdef CONFIG_OF
	struct stm_device_config *dev_config;
	void *data;

	dev_config = stm_of_get_dev_config(dev);

	BUG_ON(!dev_config);

	data = devm_stm_device_init(dev, dev_config);
	pad_config = dev_config->pad_config;

#else
	if (!devm_stm_pad_claim(dev, pad_config, owner))
		goto err;
#endif
	return 0;
err:	return -EINVAL;
...
static int stm_keyscan_probe(struct platform_device *pdev)
{
	...
	if (stm_keyscan_init_conf(dev, pdata->pad_config, pdev->name))
		return -EBUSY;
	...
	if (stm_keypad_matrix_key_parse_dt(priv))
		return -EINVAL;
...
static const struct of_device_id stm_keyscan_of_match[] = {
...
struct platform_driver stm_keyscan_device_driver = {
	...
	.driver		= {
		...
		.of_match_table = of_match_ptr(stm_keyscan_of_match),
	}
};
