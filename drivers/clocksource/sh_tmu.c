...
{
	struct sh_tmu_channel *ch = priv;
	unsigned long delta = ch->rate / rate;

	sh_tmu_set_next(ch, delta, 1);
}
...
{
	struct sh_tmu_channel *ch = NULL;
	int ret, i;
	/* As first element, look at the channel 2 because usually not
	 * used. */
	char *c[] = { "sh-tmu.2", "sh-tmu.1", "sh-tmu.0" };

	for (i = 0; i < ARRAY_SIZE(c); i++) {
		struct device *dev = bus_find_device_by_name(&platform_bus_type,
							     NULL, c[i]);
		if (dev) {
			ch = dev_get_drvdata(dev);
			if (!ch->tmu->has_clockevent && !ch->tmu->has_clocksource)
				break;
			else
				ch = NULL;
		}
	...
	if ((ch == NULL) || (ch->tm.fnc != NULL))
		return NULL;

	if (handler == NULL) {
		pr_err("sh-tmu: invalid handler\n");
		return NULL;
	}

	ch->tm.fnc = kmalloc(sizeof(struct sh_timer_callb *), GFP_KERNEL);
	if (ch->tm.fnc == NULL)
		return NULL;

	/* Prepare the new handler */
	ch->tm.priv_handler = handler;
	ch->tm.data = data;

	ret = request_irq(ch->irq, sh_timer_interrupt, 0,
			  dev_name(&ch->tmu->pdev->dev), ch);
	if (ret) {
		dev_err(&ch->tmu->pdev->dev, "sh-tmu.%u: failed to request irq %d\n",
			ch->index, ch->irq);
		return NULL;
	}
	...
	ch->tm.fnc->tmu_priv = ch; /* hook to invoke the timer callbacks */
	ch->tm.fnc->timer_start = sh_timer_start;
	ch->tm.fnc->timer_stop = sh_timer_stop;
	ch->tm.fnc->set_rate = sh_timer_set_rate;
