...
#define	TOP_EMMC_TX_CLK_DELAY_TX_CLK_MAX_DELAY		(0xf << 0)
#define	TOP_EMMC_TX_CLK_DELAY_TX_CLK_DEFAULT_DELAY	(0x0 << 0)

#define	TOP_EMMC_RX_CLK_DELAY				0x000c
#define	TOP_EMMC_RX_CLK_DELAY_RX_CLK_MAX_DELAY		(0xf << 0)
#define	TOP_EMMC_RX_CLK_DELAY_RX_CLK_DEFAULT_DELAY	(0x0 << 0)
...
#define	TOP_EMMC_DELAY_CONTROL_TX_STATIC_DELAY		(0x0 << 8)
#define	TOP_EMMC_DELAY_CONTROL_RX_DLL_ENABLE		(0x1 << 9)
#define	TOP_EMMC_DELAY_CONTROL_RX_STATIC_DELAY	(0x0 << 9)
...
#define	TX_STEP_DEFAULT_DELAY	0x6
...
/**
 * flashss_get_vsense: get voltage pad
 * Latched PSW will reflect the voltage pad only after reset or at boot time
 */
...
{
	unsigned value = readl(flashss_config + TOP_VSENSE_CONFIG);
	...
	pr_info("%s: set voltage %s (vsense_conf 0x%x)\n", __func__,
		v ? "3V3" : "1V8",
 ...
 * to perform auto-tuning on Rx path while switching on super-speed mode. It is
...
/**
+ * flashss_emmc_set_static_delay: DLL bypassed on low freq less than 90Mhz
+ * Description: this function is to extra delay in static way. The setting
+ * need some electrical measurment before identifying the right value
+ */
void flashss_emmc_set_static_delay(void)
{
	unsigned value = 0;
	...
	/* Static delay */
	value = TOP_EMMC_DELAY_CONTROL_RX_STATIC_DELAY |
		TOP_EMMC_DELAY_CONTROL_TX_STATIC_DELAY;

	writel(value, flashss_config + TOP_EMMC_DELAY_CONTROL);

	writel(TOP_EMMC_TX_CLK_DELAY_TX_CLK_MAX_DELAY, flashss_config +
	       TOP_EMMC_TX_CLK_DELAY);

	writel(TOP_EMMC_RX_CLK_DELAY_RX_CLK_DEFAULT_DELAY, flashss_config +
	       TOP_EMMC_RX_CLK_DELAY);
}
EXPORT_SYMBOL_GPL(flashss_emmc_set_static_delay);

/**
 * flashss_emmc_disable_boot: disable boot from eMMC
 * Description: by default, the eMMC managed by FlashSS is configured as
 * bootable device so this means that the HC (SDHCI) will be never able to
 * access to arasan registers. So this fuction is to move the control of the
 * MMC device from the bootsequencer to the arasan Linux HC.
 */
void flashss_emmc_disable_boot(void)
{
	unsigned value;

	BUG_ON(!flashss_config);

	value = readl(flashss_config + TOP_FLASHSS_CONFIG);

	value &= ~TOP_FLASHSS_CONFIG_CFG_EMMC_NOT_EMI;

	writel(value, flashss_config + TOP_FLASHSS_CONFIG);
}
EXPORT_SYMBOL_GPL(flashss_emmc_disable_boot);
