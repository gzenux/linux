...
static int stx7105_emi_init(struct stm_device_state *device_state)
{
	stm_device_sysconf_write(device_state, "EMI_PWR", 0);
	return 0;
}
#define stx7105_emi_power NULL
#else
#define stx7105_emi_init NULL
...
static struct platform_device stx7105_st231_coprocessor_devices[2] = {
	{
		...
		.dev.platform_data = &(struct plat_stm_st231_coproc_data) {
			...
			.device_config = &(struct stm_device_config) {
				.sysconfs_num = 2,
				.sysconfs = (struct stm_device_sysconf []) {
					STM_DEVICE_SYS_CFG(28, 8, 31, "BOOT_ADDR"),
		...
		.dev.platform_data = &(struct plat_stm_st231_coproc_data) {
			...
			.device_config = &(struct stm_device_config) {
				.sysconfs_num = 2,
				.sysconfs = (struct stm_device_sysconf []) {
					STM_DEVICE_SYS_CFG(26, 8, 31, "BOOT_ADDR"),
...
static struct platform_device *stx7105_devices[] __initdata = {
	...
	&stx7105_st231_coprocessor_devices[0],
