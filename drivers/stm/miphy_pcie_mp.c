...
struct pcie_mp_device{
	...
	void __iomem *usb3_base; /* Base for uport connected to USB3 */
				      ...
				      enum miphy_mode mode)
{
	...
	case SATA_MODE:
		...
		break;
	case USB30_MODE:
		/* Use the dedicated USB port if we have one */
		if (mp_dev->usb3_base)
			base = mp_dev->usb3_base;
		break;
	default:
		BUG();
	}
...
static int pcie_mp_probe(struct platform_device *pdev)
{
	...
	if (!mp_dev->pcie_base && !mp_dev->sata_base && !mp_dev->usb3_base) {
		dev_err(&pdev->dev, "Must have either a sata pcie or usb3 uport");
		return -EINVAL;
	}
	...
	if (data->init) {
		...
		if (config)
			mp_dev->priv_data = devm_stm_device_init(&pdev->dev,
									config);
	}
...
static int pcie_mp_pm_restore(struct device *dev)
{
	struct pcie_mp_device *mp_dev = dev_get_drvdata(dev);

	if (mp_dev->priv_data)
		stm_device_setup(mp_dev->priv_data);
	return 0;
}

static const struct dev_pm_ops pcie_mp_pm_ops = {
	.restore_noirq = pcie_mp_pm_restore,
};

#define PCIE_MP_PM_OPS		(&pcie_mp_pm_ops)
#else
#define PCIE_MP_PM_OPS		NULL
#endif
