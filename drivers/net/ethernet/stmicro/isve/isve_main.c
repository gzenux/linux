...
static int isve_alloc_rings(struct isve_priv *priv)
{
	...
	for (i = 0; i < ISVE_DEFAULT_QUEUE_SIZE; i++) {
		...
		if (netif_msg_probe(priv))
			dev_dbg(device, "dwfd - %d: buffer 0x%p addr 0x%x\n",
	...
	for (i = 0; i < ISVE_DEFAULT_QUEUE_SIZE; i++) {
		...
		if (netif_msg_probe(priv))
			dev_dbg(device, "upiim - %d: buffer 0x%p, addr 0x%x\n",
...
static int isve_pltfr_probe(struct platform_device *pdev)
{
	...
	addr_upiim = devm_request_and_ioremap(dev, res);
	...
	if (priv->irq_ds == -ENXIO) {
		dev_err(dev, "ERROR: DFWD IRQ configuration not found\n");
		ret = -ENXIO;
		goto err_free_netdev;
	}
	priv->irq_us = platform_get_irq_byname(pdev, "isveirq_us");
	if (priv->irq_ds == -ENXIO) {
		dev_err(dev, "ERROR: UPIIM IRQ configuration not found\n");
		ret = -ENXIO;
		goto err_free_irq_ds;
	}
	...
	dev_info(dev, "dfwd addr 0x%p upiim  addr 0x%p, irq_ds %d irq_us %d\n",
		 priv->ioaddr_dfwd, priv->ioaddr_upiim, priv->irq_ds,
		 priv->irq_us);
